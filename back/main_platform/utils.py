...
def expand_dataframe(df, max_depth=10, step=1, reverse=False, default_price=0):
    ...
    df_additional = pl.DataFrame(
        {"price": additional_price_elements, "amount": additional_amount_elements}
    )
...
def convert_to_book_format(active_orders, levels_n=10, default_price=2000):
    ...
    df_asks = (
        df.filter(pl.col("order_type") == OrderType.ASK.value)
        .groupby("price")
    ...
    df_bids = (
        df.filter(pl.col("order_type") == OrderType.BID.value)
        .groupby("price")
        .agg(pl.col("amount").sum())
    ...
    df_asks = expand_dataframe(
        df_asks, max_depth=10, step=1, reverse=False, default_price=default_price
    )
    df_bids = expand_dataframe(
        df_bids, max_depth=10, step=1, reverse=True, default_price=default_price - 1
    )
    ...
    interleaved_array = np.ravel(
        np.column_stack((ask_prices, ask_quantities, bid_prices, bid_quantities))
    )
...
def convert_to_book_format_new(input_data, levels_n=10, default_price=2000):
    ...
    df_bids = df_bids.with_columns(
        [pl.col("price").cast(pl.Int64), pl.col("amount").cast(pl.Int64)]
    )
    ...
    # Aggregate orders by price, summing the amounts
    df_bids = (
        df_bids.group_by("price")
        .agg(pl.col("amount").sum())
        .sort("price", descending=True)
        .head(levels_n)
    )
    df_asks = (
        df_asks.group_by("price")
        .agg(pl.col("amount").sum())
        .sort("price")
        .head(levels_n)
    )
    ...
    df_asks = expand_dataframe(
        df_asks, max_depth=levels_n, step=1, reverse=False, default_price=default_price
    )
...
def convert_to_noise_state(active_orders: List[Dict]) -> Dict:
    ...
    noise_state["outstanding_orders"]["bid"] = dict(
        noise_state["outstanding_orders"]["bid"]
    )
    noise_state["outstanding_orders"]["ask"] = dict(
        noise_state["outstanding_orders"]["ask"]
    )
