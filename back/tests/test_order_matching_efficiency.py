...
async def test_order_creation_and_processing_efficiency(capsys):
    ...
    try:
        ...
        traders = [
            BaseTrader(TraderType.NOISE, id=f"trader_{i}", cash=10000, shares=100)
        ...
        order_creation_tasks = [
            create_random_orders(trader, num_orders_per_trader) for trader in traders
        ]
        ...
        with capsys.disabled():
            ...
            print(
                f"{BLUE}Orders created and processed per second:{RESET} {GREEN}{orders_per_second:.2f}{RESET}"
            )
            print(
                f"{BLUE}Required seconds per order:{RESET} {GREEN}{total_time / total_orders:.4f}{RESET}"
            )
...
async def test_order_matching_consistency(capsys):
    ...
    try:
        ...
        trader = BaseTrader(TraderType.NOISE, id="1", cash=10000, shares=100)
        await trader.initialize()
        await trader.connect_to_session(trading_session.id)
