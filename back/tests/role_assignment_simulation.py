...
def simulate_fixed_traders():
    ...
    def validate_conditions():
        ...
        for session_id, session in sessions.items():
            if session.state == SessionState.ACTIVE:
                informed_count = sum(1 for uid in session.traders 
                                  if users[uid].role == RoleType.INFORMED)
                if informed_count != 1:
                    violations.append(
        ...
        for user_id, user in users.items():
            if user.role is not None:  # Skip unassigned users
                ...
                sessions_participated = user.historical_sessions | {user.current_session} if user.current_session else user.historical_sessions
    ...
    while current_time < max_time:
        ...
        for session in list(sessions.values()):
            if (session.state == SessionState.ACTIVE and 
                current_time >= session.start_time + session.duration):
                terminate_session(session.id)
    ...
    if not final_violations:
        ...
        print(f"Active sessions: {sum(1 for s in sessions.values() if s.state == SessionState.ACTIVE)}")
        ...
        for user_id, user in users.items():
            ...
            if len(user.historical_sessions) > 0:
                print(f"  Average sessions per 1000 time units: {len(user.historical_sessions) * 1000 / max_time:.2f}")
    else:
