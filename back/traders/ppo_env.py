...
class TradingEnvironment(gym.Env):
    def __init__(self, trading_session):
        ...
        self.previous_portfolio_value = self.trading_session.cash + self.trading_session.shares * self.trading_session.mid_price
        
        # Define observation space
        self.observation_space = spaces.Dict({
            'order_book': spaces.Dict({
                'best_bid': spaces.Box(low=0, high=np.inf, shape=(2,)),  # price, volume
                'best_ask': spaces.Box(low=0, high=np.inf, shape=(2,)),  # price, volume
                'depth': spaces.Box(low=0, high=np.inf, shape=(10, 2)),  # 5 levels each side
            }),
            ...
            'trader_state': spaces.Dict({
                ...
                'unrealized_pnl': spaces.Box(low=-np.inf, high=np.inf, shape=(1,)),
            }),
            'time': spaces.Dict({
    ...
    def _get_observation(self):
        ...
        obs = {
            'order_book': {
                'best_bid': np.array([order_book['bids'][0]['price'], order_book['bids'][0]['amount']]),
                'best_ask': np.array([order_book['asks'][0]['price'], order_book['asks'][0]['amount']]),
                'depth': np.array([[level['price'], level['amount']] for level in order_book['bids'][:5] + order_book['asks'][:5]]),
            },
            'transactions': {
            ...
            'time': {
                ...
                'remaining': np.array([self.trading_session.duration * 60 - self.trading_session.get_elapsed_time()]),
            },
        }
        return obs
    ...
    def _calculate_reward(self):
        ...
        reward = portfolio_value_change - transaction_cost - holding_penalty - inventory_penalty
        return reward
    ...
    def _calculate_unrealized_pnl(self):
        ...
        return self.trading_session.shares * (current_price - self.trading_session.average_entry_price)

    def _calculate_portfolio_value_change(self):
        current_value = self.trading_session.cash + self.trading_session.shares * self.trading_session.mid_price
    ...
    def _calculate_holding_penalty(self):
        return 0.0001 * abs(self.trading_session.shares * self.trading_session.mid_price)

    def _calculate_inventory_penalty(self):
        time_remaining = self.trading_session.duration * 60 - self.trading_session.get_elapsed_time()
    ...
    def step(self, action):
        ...
        done = self.trading_session.get_elapsed_time() >= self.trading_session.duration * 60
