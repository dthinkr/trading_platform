...
# Session management for waiting room
waiting_room_users = {}  # {session_id: [user_list]}
session_counter = 0
...
async def join_waiting_room(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Get trading parameters to know how many players needed
        ...
        # Find or create a session that needs players
        session_id = None
        for sid, users in waiting_room_users.items():
            if len(users) < required_players:
                session_id = sid
                break
        ...
        if not any(u["username"] == gmail_username for u in waiting_room_users[session_id]):
            waiting_room_users[session_id].append(user_info)
            print(f"Added {gmail_username} to {session_id}. Players: {len(waiting_room_users[session_id])}/{required_players}")
        
        # Check if session is ready to start
        session_ready = len(waiting_room_users[session_id]) >= required_players
        
        if session_ready:
            # Assign all users in this session to a trading market
            try:
                market_id, assigned_traders = await assign_session_to_market(session_id, waiting_room_users[session_id], params)
                print(f"Session {session_id} ready! Assigned to market {market_id}")
                
                # Remove session from waiting room
                del waiting_room_users[session_id]
                
                # Find this user's trader assignment
                ...
                print(f"Error assigning session to market: {str(e)}")
                return {
                    "status": "error",
            ...
            return {
                "status": "success",
                "message": f"Waiting for more players ({len(waiting_room_users[session_id])}/{required_players})",
                "data": {
                    "session_ready": False,
                    "session_id": session_id,
                    "current_players": len(waiting_room_users[session_id]),
                    "required_players": required_players,
                    "waiting_for": required_players - len(waiting_room_users[session_id])
                }
            }
...
async def assign_session_to_market(session_id: str, users: list, params: TradingParameters):
    """Assign all users in a session to a single trading market"""
    import uuid
    
    # Create unique market ID for this session
    market_id = f"MARKET_{session_id}_{uuid.uuid4().hex[:8]}"
    ...
    for i, user in enumerate(users):
        ...
        # For now, all users get the same role (can be enhanced later)
        role = "HUMAN"
        
        # Use the existing market assignment logic
        market_id_assigned, trader_id_assigned, role_assigned, goal_assigned = await market_handler.validate_and_assign_role(
            username, params, preferred_market_id=market_id, preferred_goal=goal
        )
        ...
        assigned_traders.append({
            ...
            "role": role_assigned,
            "goal": goal_assigned,
            "is_prolific": user.get("is_prolific", False)
        })
        
        print(f"Assigned {username} to market {market_id_assigned} with goal {goal_assigned}")
...
async def get_waiting_room_status(current_user: dict = Depends(get_current_user)):
    ...
    for session_id, users in waiting_room_users.items():
        if any(u["username"] == gmail_username for u in users):
            params = TradingParameters(**(persistent_settings or {}))
            required_players = len(params.predefined_goals)
            ...
            return {
                ...
                "data": {
                    ...
                    "required_players": required_players,
                    "waiting_for": required_players - len(users),
