...
def load_problem_and_sessions():
    latest_file = max([f for f in os.listdir(CONFIG.DATA_DIR) if f.startswith('trader_data_mapping') and f.endswith('.json')], key=lambda f: os.path.getmtime(os.path.join(CONFIG.DATA_DIR, f)))
...
def plot_sobol_indices(time_range, Si, problem, date_str):
    ...
    for i, param in enumerate(problem['names']):
        ...
        plt.plot(time_range, Si['ST'][:, i], label=f'{param} (Total Order)', linestyle='--')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(CONFIG.DATA_DIR, f'figs/sobol_sensitivity_indices_{date_str}.png'))
    plt.close()
...
def export_sobol_indices(Si, problem, date_str):
    ...
    with open(os.path.join(CONFIG.DATA_DIR, f'sobol_indices_{date_str}.json'), 'w') as f:
